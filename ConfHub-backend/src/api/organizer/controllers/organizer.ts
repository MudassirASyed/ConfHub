...
async updateReviewFormFields(ctx) {
 ...
 async assignReviewersToPaper(ctx) {
  const { paperId, reviewers = []} = ctx.request.body;

  console.log('Request Body:', ctx.request.body);

  if (!paperId || reviewers.length === 0 ) {
...
//   }
 ...
 async sendOtp(ctx) {
  ...
  try {
    ...
    const FRONTEND_URL = "https://www.bzchair.org";
    const invitations = [];
    ...
    for (const reviewerId of existingReviewerIds) {
      ...
      // 🟢 Email links
     const acceptUrl = `https://bzchair-backend.up.railway.app/api/reviewer-invitation/respond?token=${token}&action=accept`;
const rejectUrl = `https://bzchair-backend.up.railway.app/api/reviewer-invitation/respond?token=${token}&action=reject`;
...
`;
      ...
      const htmlBody = `
        ...
        <p>You are invited to review the following conference on BZChair:</p>
        <p>Conference: "${conf.Conference_title}"</p>
    ...
    //  if (existingReviewer) reviewerId = existingReviewer.id;
      ...
      const acceptLink = `https://bzchair-backend.up.railway.app/api/reviewer-invitation/respond?token=${token}&action=accept`;
      const rejectLink = `https://bzchair-backend.up.railway.app/api/reviewer-invitation/respond?token=${token}&action=reject`;
