...
const ManageInvitations = () => {
  ...
  useEffect(() => {
    const fetchInvitationsData = async () => {
      try {
        ...
        const activeResponse = await axios.get('https://bzchair.org/api/invitations/active');
        setActiveInvitations(activeResponse.data);

        const pendingResponse = await axios.get('https://bzchair.org/api/invitations/pending');
  ...
  const handleApprove = async (invitationId) => {
    try {
      ...
      await axios.post(`https://bzchair.org/api/invitations/approve/${invitationId}`);
